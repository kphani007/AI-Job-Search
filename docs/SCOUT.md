...
This is the single source of truth for the daily InsurTech/retirement job digest
routine. It replaces logic that previously existed only inside the routine's
prompt. A fresh agent with no prior context should be able to run a complete,
correct digest from this document alone.

Reverse-engineered from `seen-jobs.md` and `docs/index.html` history as of
2026-08-14. If reality and this document ever disagree, update this document
in the same PR — don't let the logic drift back into an undocumented prompt.

## 1. Goal

Track new Project/Delivery Management and Quality/QA management job postings
in the insurance technology (InsurTech), life/P&C insurance, and retirement
sectors, primarily in India, for a Project Manager job seeker. One run per
day, append-only.
...
- **PM / Delivery** (`category: "pm"`): Project Manager, Technical Project
  ...
  Associate Project Manager, PMO roles.
- **QA / Quality** (`category: "qa"`): QA Manager, Quality Assurance Manager,
  Quality Management Manager, Test Manager.

Seniority: Associate through Senior Manager / Director. Exclude individual
contributor roles with no delivery/PM/QA-management scope (e.g. plain QA
Analyst/Tester, plain Business Analyst) and exclude VP+ / C-level roles.
...
No fixed company allowlist. Sector filter instead:
...
A posting must reference the insurance or retirement domain in the title,
company description, or job summary. General "Project Manager" postings with
no insurance/retirement domain signal are out of scope.

Geographic scope: India-wide, any city. (All postings logged so far are
India-based; don't hard-exclude a genuinely strong remote/global InsurTech PM
role if one turns up, but India is the default search geography.)

## 4. Sources

WebFetch to job-board domains (linkedin.com, jobaaj.com, naukri.com,
indeed.com, etc.) is blocked by this environment's network egress policy —
confirmed for every job-board domain tested on 2026-08-14 (see §7). **Do not
rely on WebFetch for job boards.** Use the `WebSearch` tool with a
`site:<domain>` filter instead — confirmed working and returns individual
job-posting URLs with title/company/location snippets, which is enough to
build a `seen-jobs.md` row without ever fetching the page body.

| # | Source | Query method | Example query |
|---|---|---|---|
| 1 | LinkedIn Jobs (primary) | `WebSearch`, `site:linkedin.com/jobs` or `site:in.linkedin.com/jobs` | `site:linkedin.com/jobs project manager insurance India` |
| 2 | Jobaaj (secondary / LinkedIn mirror) | `WebSearch`, `site:jobaaj.com` | `site:jobaaj.com quality assurance manager insurance India` |
| 3 | Naukri (secondary) | `WebSearch`, `site:naukri.com` | `site:naukri.com delivery manager life insurance` |
| 4 | Indeed (secondary) | `WebSearch`, `site:indeed.com` or `site:in.indeed.com` | `site:in.indeed.com project manager insurance` |
| 5 | GitHub issues on this repo (manual leads) | `mcp__github__list_issues` (state OPEN) on `kphani007/AI-Job-Search` | — |

Run at least sources 1 and 5 every day; run 2–4 as supplementary sweeps to
widen coverage. Rotate/vary the exact query keywords run-to-run (role terms ×
sector terms) rather than repeating one fixed query, to surface postings a
single fixed query would miss.
...
Include a posting if **all** of the following hold:

1. Title matches §2 (PM/Delivery or QA/Quality bucket).
2. Domain matches §3 (insurance/retirement sector signal present).
3. Within the recency window (§6).
4. Not already in `seen-jobs.md` per the dedupe key (§8).

Exclude and do not log:

- Pure IC roles with no management/delivery/QA-lead scope.
- Postings with no verifiable post date where the source itself flags the
  listing as old (see the Allianz Technology case below).
- Postings outside India with no strong remote-eligibility signal for an
...
`seen-jobs.md` row, "Rejected - stale posting", and commit
`8862f62`). When a source shows a relative date ("Xd ago", "posted today")
without an absolute date, treat it as unverified — cross-check against the
canonical listing (e.g. the LinkedIn URL itself) before trusting it, and if
it can't be verified, log it as `Status: New` but flag it in the run log as
`unverified date` rather than silently including or excluding it.

## 6. Recency window

**30 days.** A posting is in-window if its actual post date (not the date a
mirror site indexed it) is within the last 30 days of the run date. This is
inferred from the Allianz rejection (a ~1-year-old posting was explicitly
too stale) — no run has yet had to draw a line between, say, 20 and 40 days,
so treat 30 as the working default and tighten/loosen it here if the user
says otherwise.
...
A posting is a duplicate of an existing `seen-jobs.md` row if either matches:
...
First Seen" — the date only reflects the first time this routine saw it.
...
- **Source**: `LinkedIn`, `Jobaaj (LinkedIn-sourced)`, `Naukri`, `Indeed`, or
  `<Original Source> (GitHub issue lead)` for §4 source 5.
- **Status**: `New` for anything included per §5. If later found to be
...
1. Rebuild the `ALL_JOBS` JS array from every `seen-jobs.md` row whose Status
   is `New` (exclude `Rejected - *` rows). Each entry:
